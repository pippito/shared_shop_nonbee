...
/**
 * 注文管理 一覧表示機能(一般ユーザ用)のコントローラクラス
 *
 * @author SystemShared
 */
@Controller
public class OrderShowUserController {
	 ...
	 */
	@RequestMapping(path = "/order/list/user", method = RequestMethod.GET)
	public String showOrderList(Model model, @ModelAttribute OrderShowForm form) {
		...
		model.addAttribute("url", "/order/list/user");

		return "order/list/order_list_user";
	 ...
	 * @param session
	 *            セッション情報
	 * @return "/order/detail/order_detail_user" 注文情報 詳細画面へ
	 */
	@RequestMapping(path = "/order/detail/user/{id}")
	public String showOrder(@PathVariable int id, Model model, @ModelAttribute OrderShowForm form) {
		...
		return "order/detail/order_detail_user";
	}
